...
export class ContestProblemFilterStrategy extends ProblemsPaginationService {
  constructor(
    ...
    super(
      ...
      new Logger(ContestProblemFilterStrategy.name),
    );
  }
  ...
  protected buildFilterChain(
    ...
    const permissionFilter = new CompositeFilter(
      LogicalOperator.AND,
    ).addFilters(
      new TypeFilter(alias),
      new CompositeFilter(LogicalOperator.OR).addFilters(
